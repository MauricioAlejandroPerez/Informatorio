...
# Si no indicamos el valor a retornar la función(no escribimos return) nos devuelve None por defecto. 
# None representa a un valor vacio
def una_funcion():
...
# Los valores mutables se comportan como paso por referencia ya que conservan los cambios hechos 
...
#Al trabajar con valores inmutables se comportan como paso por valor ya que conservan su valor original 
# sin importar los cambios y asignaciones que se hagan dentro de la funcion.
...
# Al trabajar con valores mutables se comportan como paso por valor 
# ya que conservan ya que conservan los cambios hechos dentro de la funcion.
# Si vemos el ejemplo tendremos el mismo valor dentro y fuera de la funcion 
# ya que mantiene los cambios realizados al ejecutar la funcion.
...
# Si necesitamos modificar el valor de la función tendrá un valor dependiendo de donde 
# se la llame ya que depende del contexto (namespace)
animal = "León"                         # Es una variable GLOBAL
...
# En el primer ejemplo al llegar a la letra h no se ejecuta nada (se pasa), 
# siguiendo con la ejecución de la siguiente linea (la linea 7). 
